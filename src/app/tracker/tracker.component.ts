...
export class TrackerComponent implements OnInit {
  ...
  websocketIsSetUp: boolean = false;
  ...
  async ngOnInit(): Promise<void> {
    ...
    // const ws = new WebSocket("ws://127.0.0.1:5200/output");
    // const ws = new WebSocket("wss://www.example.com/socketserver");
    const ws = io("ws://localhost:5200/output");

    ws.on("connect", () => {
      console.log("Connected to websocket");

      ws.on("message", (messageEvent: MessageEvent) => {
        const data = JSON.parse(messageEvent.data);
        if (this.websocketIsSetUp) {
          this.updateMatch(data);
        }
        else {
          console.log(data.msg);
          this.websocketIsSetUp = true;
        }
      });
